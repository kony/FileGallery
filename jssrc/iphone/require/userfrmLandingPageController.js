define({
    ...
    helpClicked: false,
    onNavigate: function() {
        if (flag === false) stopAudio();
        for (var sections in this.collectionViewData) {
            for (var rows in this.collectionViewData[sections][1]) {
                if (kony.os.deviceInfo().name === "android") this.collectionViewData[sections][1][rows]["flxCollection"]["width"] = "49.7%";
                else if (kony.os.deviceInfo().name === "iPhone") this.collectionViewData[sections][1][rows]["flxCollection"]["width"] = "50%";
            }
        }
        this.addViewToForm();
